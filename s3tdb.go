...
func getCachedBlock(req extPacket, s3conn *s3.S3, buffer []byte) (extPacket, error) {
	var loader = func(dir string,
		fname string,
		url string,
		offset uint64,
		maxSize uint64) (uint64, error) {
...
func handshake(req extPacket, s3conn *s3.S3, buffer []byte) extPacket {
	if req.path[:5] != "s3://" {
